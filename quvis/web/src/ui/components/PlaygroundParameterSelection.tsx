...
const PlaygroundParameterSelection: React.FC<PlaygroundParameterSelectionProps> = ({ onGenerate }) => {
    const [algorithm, setAlgorithm] = useState<string>("qft");
    const [logicalQubits, setLogicalQubits] = useState<number>(6);
    ...
    const circuitButtonStyle = (isActive: boolean): React.CSSProperties => ({
        ...
        borderRadius: "12px",
        backgroundColor: isActive ? colors.primary.accent : colors.background.panelSolid,
    ...
    const generateButtonStyle: React.CSSProperties = {
        ...
        fontSize: "24px",
        fontWeight: "bold",
        backgroundColor: isGenerating ? colors.background.panel : colors.primary.accent,
        ...
        boxShadow: isGenerating ? "none" : `0 4px 20px ${colors.primary.accent}40`,
    };
    ...
    const handleLogicalQubitsInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const value = parseInt(e.target.value);
        if (!isNaN(value) && value >= 4 && value <= MAX_QUBITS) {
            setLogicalQubits(value);
        }
    };

    const handlePhysicalQubitsInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const value = parseInt(e.target.value);
        if (!isNaN(value) && value >= logicalQubits && value <= MAX_QUBITS) {
            setPhysicalQubits(value);
        }
    };
                ...
                <div style={qaoacParamsStyle}>
                    ...
                    <div style={sliderWrapperStyle}>
                        <input
                            ...
                            onChange={(e) => handleCustomParamChange("reps", parseInt(e.target.value))}
                            style={sliderStyle}
                        />
                        <input
                            type="number"
                ...
                <div style={columnStyle}>
                    ...
                    <div style={sectionStyle}>
                        ...
                        <div style={buttonGroupStyle}>
                            ...
                            <button
                                style={topologyButtonStyle(topology === "heavy_square")}
                                onClick={() => setTopology("heavy_square")}
                            >
                                Heavy Square
                            </button>
                            <button
                                style={topologyButtonStyle(topology === "hexagonal")}
                        ...
                        <div style={sliderContainerStyle}>
                            ...
                            <div style={sliderWrapperStyle}>
                                <input
                                    ...
                                    onChange={(e) => setPhysicalQubits(parseInt(e.target.value))}
                                    style={sliderStyle}
                                />
                                <input
                                    type="number"
                    ...
                    <div style={sectionStyle}>
                        <div style={sliderContainerStyle}>
                            <div style={sliderLabelStyle}>Optimization Level</div>
                            <select
                                value={optimizationLevel}
                                onChange={(e) => setOptimizationLevel(parseInt(e.target.value))}
                                style={selectStyle}
                            >
                                <option value={0}>Level 0 - No optimization</option>
                                <option value={1}>Level 1 - Light optimization</option>
                                <option value={2}>Level 2 - Heavy optimization</option>
                                <option value={3}>Level 3 - Max optimization</option>
                            </select>
                        </div>
                    </div>
                </div>
