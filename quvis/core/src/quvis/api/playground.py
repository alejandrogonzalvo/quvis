...
class PlaygroundAPI:
    ...
    def generate_visualization_data(self, 
        ...
        print(f"📊 Processing circuit for playground visualization...", file=sys.stderr)
        
        # Process logical circuit
        print(f"   🧠 Processing logical circuit...", file=sys.stderr)
        logical_circuit_data = self._process_logical_circuit(circuit, algorithm, num_qubits, topology, kwargs)
        
        # Process compiled circuit
        print(f"   🔗 Processing compiled circuit...", file=sys.stderr)
        compiled_circuit_data = self._process_compiled_circuit(
            circuit, coupling_map, qiskit_coupling_map, basis_gates, 
            optimization_level, algorithm, num_qubits, topology, kwargs
        )
        
        # Return in library_multi format (same as library mode)
        result = {
        ...
        print(f"✅ Playground circuit generation completed successfully!", file=sys.stderr)
        print(f"   📊 Generated logical and compiled versions", file=sys.stderr)
        
        return result
    
    def _process_logical_circuit(self, circuit: QuantumCircuit, algorithm: str, 
                               num_qubits: int, topology: str, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(f"   ✓ Extracted {len(logical_operations_per_slice)} time slices from logical circuit", file=sys.stderr)
    ...
    def _process_compiled_circuit(self, circuit: QuantumCircuit, coupling_map: Dict[str, Any],
                                qiskit_coupling_map, basis_gates: List[str], optimization_level: int,
                                algorithm: str, num_qubits: int, topology: str, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        """Process the compiled version of the circuit."""
        print(f"🔧 Transpiling for optimization level {optimization_level}...", file=sys.stderr)
        transpile_options = {
        ...
        print(f"   ✓ Transpilation complete: {len(transpiled_circuit.data)} gates total", file=sys.stderr)
        
        compiled_operations_per_slice = extract_operations_per_slice(transpiled_circuit)
        print(f"   ✓ Extracted {len(compiled_operations_per_slice)} time slices from compiled circuit", file=sys.stderr)
        
        routing_operations_per_slice, total_swap_count, routing_depth = extract_routing_operations_per_slice(transpiled_circuit)
        print(f"   ✓ Found {total_swap_count} SWAP gates for qubit routing", file=sys.stderr)
        
        routing_analysis = analyze_routing_overhead(circuit.decompose(), transpiled_circuit)
        print(f"   ✓ Routing overhead: {routing_analysis['routing_overhead_percentage']:.1f}%", file=sys.stderr)
    ...
    def _create_circuit(self, algorithm: str, num_qubits: int, **kwargs) -> QuantumCircuit:
    ...
    def _create_coupling_map(self, topology: str, num_qubits: int) -> Dict[str, Any]:
        ...
        if topology == "line":
            ...
            raise ValueError(f"Unsupported topology: {topology}. Available: {self.get_supported_topologies()}")
    ...
    def get_supported_topologies(self) -> list:
        ...
        return ["line", "ring", "grid", "heavy_hex", "heavy_square", "hexagonal", "full"]


# Convenience function for direct usage
def generate_playground_circuit(algorithm: str, num_qubits: int, topology: str, **kwargs) -> Dict[str, Any]:
    """
