...
export class npzLoader {
    ...
    parseNPZ(data) {
        ...
        let offset = 0;
        console.log(
            "[npzLoader] Starting NPZ parse (uncompressed). Data length:",
            data.length,
        );
        let fileCount = 0;
        ...
        while (offset < data.length - 30) {
            ...
            if (!header) {
                console.log(
                    "[npzLoader] No more valid ZIP headers or invalid signature at offset:",
                    offset,
                );
                break;
            }
            ...
            if (filename.endsWith(".npy")) {
                ...
                if (entryDataStart > data.length) {
                    console.error(
                        `[npzLoader] Calculated entryDataStart ${entryDataStart} for ${filename} exceeds data length ${data.length}. Skipping.`,
                    );
                    offset +=
                        30 +
                        header.filenameLength +
                        header.extraLength +
                        Math.max(0, header.compressedSize);
                    continue;
                }
                ...
                if (header.compression !== 0) {
                    console.warn(
                        `[npzLoader] Expected compression method 0 (Stored) but got ${header.compression} for ${filename}. Will attempt to read uncompressedSize anyway.`,
                    );
                }
                if (
                    header.compressedSize !== header.uncompressedSize &&
                    header.compressedSize !==
                        0xffffffff /*ignore Zip64 marker if it appears*/
                ) {
                    console.warn(
                        `[npzLoader] For Stored file ${filename}, compressedSize (${header.compressedSize}) != uncompressedSize (${header.uncompressedSize}). Using uncompressedSize.`,
                    );
                }
                ...
                if (npyFileLength > availableDataForEntry) {
                    console.error(
                        `[npzLoader] NPY file length ${npyFileLength} for ${filename} (from uncompressedSize) exceeds available data ${availableDataForEntry}. Skipping.`,
                    );
                    offset +=
                        30 +
                        header.filenameLength +
                        header.extraLength +
                        header.compressedSize; // Use original compressedSize for offset advancement
                    continue;
                }

                let npyFileBytes = data.subarray(
                    entryDataStart,
                    entryDataStart + npyFileLength,
                );

                console.log(
                    `[npzLoader] NPY data buffer for ${filename} has length: ${npyFileBytes.length}`,
                );
                ...
                try {
                    ...
                    console.log(
                        `[npzLoader] Successfully parsed and stored: ${filename.replace(".npy", "")}`,
                    );
                } catch (e) {
                    console.error(
                        `[npzLoader] Error parsing NPY content for ${filename}:`,
                        e,
                    );
                }
            } else {
        ...
        if (fileCount === 0 && data.length > 0) {
            console.warn(
                "[npzLoader] Parsed NPZ, but found no ZIP entries. Is the file a valid ZIP archive?",
            );
        }
    ...
    parseNpy(data, filename) {
        ...
        if (data.length < 10)
            throw new Error(
                `NPY data too short for header (length: ${data.length}) for ${filename || "unknown file"}`,
            );

        const magic = String.fromCharCode.apply(null, data.subarray(0, 6));
        if (magic !== "\x93NUMPY")
            throw new Error(
                `Invalid NPY magic string for ${filename || "unknown file"}`,
            );

        const major = data[6];
        if (major !== 1 && major !== 2 && major !== 3)
            throw new Error(
                `Unsupported NPY major version ${major} for ${filename || "unknown file"}`,
            );
        ...
        if (headerStartOffset + headerLength > data.length) {
            throw new Error(
                `Calculated NPY header length (${headerLength}) from offset ${headerStartOffset} exceeds NPY data segment (length: ${data.length}) for ${filename || "unknown file"}.`,
            );
        }

        const headerStrBytes = data.subarray(
            headerStartOffset,
            headerStartOffset + headerLength,
        );
    ...
    parseNpyHeader(headerStr, filename) {
        console.log(
            `[npzLoader] NPY header string for ${filename || "unknown file"} before replacements:`,
            headerStr,
        );
        ...
        console.log(
            `[npzLoader] NPY header string for ${filename || "unknown file"} after replacements (JSON candidate):`,
            jsonCandidate,
        );
        ...
        try {
            ...
            console.error(
                `[npzLoader] JSON.parse failed for ${filename || "unknown file"}. Candidate string was:`,
                jsonCandidate,
                "Original error:",
                e,
            );
            throw new Error(
                `Failed to parse NPY header for ${filename} (after replacements): ${e.message}. Original header: <${headerStr}>. Processed header: <${jsonCandidate}>`,
            );
        }
    }

    parseNpyData(data, header, filename) {
        const dtypeMatch = header.descr.match(
            /[|<>]?(u?)(int|float|S|U|O|b|bool|datetime64|timedelta64)([\d_]*)/i,
        );
        if (!dtypeMatch)
            throw new Error(
                `Unsupported data type description '${header.descr}' in NPY header for ${filename || "unknown file"}`,
            );
        ...
        if (typeCategory.toUpperCase() === "O") {
            ...
            console.warn(
                `[npzLoader] Encountered dtype 'object' (O) for ${filename || "unknown file"}. Expecting a string type (<U or |S) for JSON payload. Attempting to treat as string if shape is () or (1,).`,
            );
        ...
        if (
            typeCategory.toUpperCase() === "S" ||
            typeCategory.toUpperCase() === "U"
        ) {
            const itemLength = parseInt(typeLengthStr) || 1;
            if (itemLength === 0)
                throw new Error(
                    `String type ${typeCategory} has itemLength 0 for ${filename || "unknown file"}`,
                );

            const numItems = Math.floor(data.byteLength / itemLength); // Use Math.floor for safety
            if (
                data.byteLength % itemLength !== 0 &&
                !(typeCategory.toUpperCase() === "S" && itemLength === 1)
            ) {
                console.warn(
                    `[npzLoader] String data bytelength (${data.byteLength}) for ${filename || "unknown file"} is not a multiple of item length (${itemLength}). Might parse ${numItems} full items.`,
                );
            }
            const strings = [];
            const decoder =
                typeCategory.toUpperCase() === "U"
                    ? new TextDecoder("utf-32le")
            ...
            for (let i = 0; i < numItems; i++) {
                ...
                if (typeCategory.toUpperCase() === "U") {
                    ...
                    for (let k = 0; k < itemLength / 4; k++) {
                        if (
                            stringBytesRaw[k * 4] === 0 &&
                            stringBytesRaw[k * 4 + 1] === 0 &&
                            stringBytesRaw[k * 4 + 2] === 0 &&
                            stringBytesRaw[k * 4 + 3] === 0
                        ) {
                            break;
                        }
                        actualCharLength++;
                    }
                    stringBytesProcessed = stringBytesRaw.subarray(
                        0,
                        actualCharLength * 4,
                    );
                } else {
            ...
            console.log(
                `[npzLoader] Parsed ${strings.length} strings for ${filename || "unknown file"}. First item:`,
                strings.length > 0
                    ? strings[0].substring(0, 100) + "..."
                    : "N/A",
            );
        ...
        if (isNaN(bits) && (typeCategory === "bool" || typeCategory === "b"))
            bits = 8;

        const bytes = bits / 8;
        if (bytes === 0 && !(typeCategory === "bool" || typeCategory === "b")) {
            throw new Error(
                `Calculated 0 bytes per element for dtype '${header.descr}' in ${filename || "unknown file"}`,
            );
        }
        ...
        if (!constructor) {
            throw new Error(
                `Failed to find a TypedArray constructor for dtype '${header.descr}', typeCat '${typeCategory}', bytes ${bytes} for ${filename || "unknown file"}`,
            );
        }

        if (data.byteLength === 0) {
            // Handle empty data array for a numeric type
            console.warn(
                `[npzLoader] Numerical data array for ${filename} is empty (0 bytes). Returning empty typed array.`,
            );
            return new constructor(0);
        }
        if (bytes === 0 && data.byteLength > 0) {
            // Avoid division by zero if bytes is unexpectedly zero but data exists
            throw new Error(
                `Element size is 0 bytes but data array is not empty for ${filename}. Dtype: ${header.descr}`,
            );
        }
        if (data.byteLength % bytes !== 0) {
            console.warn(
                `[npzLoader] Numerical data bytelength (${data.byteLength}) for ${filename || "unknown file"} is not a multiple of element size (${bytes} bytes) for dtype '${header.descr}'. Data might be truncated or misaligned.`,
            );
        }
        ...
        return new constructor(
            data.buffer,
            data.byteOffset,
            Math.floor(data.byteLength / bytes),
        ); // Use Math.floor for num elements
    }
    ...
    getTypedArrayConstructor(type, bytes, unsigned) {
        ...
        console.warn(
            `[npzLoader] No TypedArray constructor found for type: ${type}, bytes: ${bytes}, unsigned: ${unsigned}`,
        );
        return null;
    }
}
