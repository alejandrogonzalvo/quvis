...
export class npzLoader {
    ...
    parseNPZ(data) {
        ...
        while (offset < data.length - 30) {
            ...
            if (!header)
                break;
            const filename = this.readFilename(data, offset + 30, header.filenameLength);
            if (filename.endsWith('.npy')) {
                const start = offset + 30 + header.filenameLength + header.extraLength;
    ...
    parseZipHeader(data, offset) {
        ...
        if (signature !== 0x04034b50)
            return null; // Local file header signature
        return {
    ...
    parseNpy(data) {
        const view = new DataView(data.buffer);
        const magic = String.fromCharCode(...new Uint8Array(data.subarray(0, 6)));
        if (magic !== '\x93NUMPY')
            throw new Error('Invalid NPY magic string');
        const major = data[6];
        const minor = data[7];
        if (major !== 1 && major !== 2)
            throw new Error('Unsupported NPY version');
        const headerLength = major === 1 ? view.getUint16(8, true) : view.getUint32(8, true);
        const headerStr = String.fromCharCode(...data.subarray(10, 10 + headerLength));
    ...
    parseNpyData(data, header) {
        ...
        const constructor = this.getTypedArrayConstructor(type, parseInt(bits) / 8, unsigned);
        return new constructor(data.buffer, data.byteOffset, data.byteLength / (parseInt(bits) / 8));
    }
